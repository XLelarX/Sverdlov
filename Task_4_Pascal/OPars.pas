...
begin
   ...
   if Lex = lexNum then begin
      ...
      if X^.Cat = catGuard then
         Error('Нельзя определить константу через нее саму')
...
(* Простое Выраж [Отношение ПростоеВыраж] *)
...
begin
   ...
   then begin
      ...
      if T <> typInt then
         Error('Несоответствие операции типу операнда');
      NextLex;
      SimpleExpr(T); {Правый операнд отношение}
      if T <> typInt then
         Expected('выражение целого типа');
      GenComp(Op);   {Генерация условного перехода}
      T := typBool;
   end; {иначе тип равен типу первого простого выражение}
end;
